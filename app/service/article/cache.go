...
func (a *articleService) WarmUpCache(ctx context.Context) error {
	...
	// 从 MySQL 列出全部已发布文章的 ID、创建时间和浏览量
	...
	// 预热文章 ZSet 缓存
	if err = a.replaceArticleOrderCaches(ctx, list, true, true); err != nil {
		a.logger.Error("failed to warm up article cache", zap.Error(err))
		return err
	}
	return nil
}

// replaceArticleOrderCaches 使用 MySQL 快照原子替换指定的文章排序 ZSet。
// list 是全部已发布文章的 ID、创建时间和浏览量；replaceTime、replaceView
// 分别表示是否重建时间索引和浏览量索引。临时 Key 设置一小时 TTL 只用于
// 异常清理，RENAME 成正式 Key 后会通过 PERSIST 立即移除 TTL。
func (a *articleService) replaceArticleOrderCaches(ctx context.Context, list []article.TimeAndViewZSet, replaceTime, replaceView bool) error {
	if !replaceTime && !replaceView {
		return nil
	}
	var err error
	...
	if replaceTime {
		tempKeys = append(tempKeys, tempTimeKey)
	}
	if replaceView {
		tempKeys = append(tempKeys, tempViewKey)
	}
	defer func() {
		cleanupCtx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
		defer cancel()
		if cleanupErr := a.redis.Del(cleanupCtx, tempKeys...).Err(); cleanupErr != nil {
			a.logger.Warn("failed to clean temporary article cache", zap.String("time_key", tempTimeKey), zap.String("view_key", tempViewKey), zap.Error(cleanupErr))
		}
	}()
	...
	for start := 0; start < len(list); start += warmUpBatchSize {
		...
		if _, err = pipe.Exec(ctx); err != nil {
			a.logger.Error("failed to warm up article ZSet", zap.Int("start", start), zap.Int("end", end), zap.Error(err))
			return fmt.Errorf("failed to warm up article ZSet: %w", err)
		}
	}

	// RENAME、TTL 清理与哨兵清理在同一个 Redis 事务中执行。RENAME 会继承
	// 临时 Key 的 TTL，因此必须显式 PERSIST，否则正式 ZSet 会在一小时后过期。
	if _, err = a.redis.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
		if replaceTime {
			pipe.Rename(ctx, tempTimeKey, timeKey)
			pipe.Persist(ctx, timeKey)
			pipe.ZRem(ctx, timeKey, sentinel)
		}
		if replaceView {
			pipe.Rename(ctx, tempViewKey, viewKey)
			pipe.Persist(ctx, viewKey)
